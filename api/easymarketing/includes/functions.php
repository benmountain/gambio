...
function mod_get_brand($manufacturers_id, $brand = '')
{
	if(!empty($brand))
	{
		return $brand;
	...
	$select = xtc_db_query("SELECT manufacturers_name FROM manufacturers WHERE manufacturers_id = '".$manufacturers_id."'");
...
{
	$tables = mod_get_all_db_tables();
	
	if(!in_array('products_item_codes', $tables))
	{
		return array();
	}
